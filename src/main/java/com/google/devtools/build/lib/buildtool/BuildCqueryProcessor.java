...
/**
 * {@link BuildTool.AnalysisPostProcessor} for {@code build --cquery}: evaluates a cquery
 * expression over the post-analysis configured-target graph and returns a filtered {@link
 * AnalysisResult} containing only the matched configured targets.
 *
 * <p>Unlike {@link CqueryProcessor} (used by the {@code cquery} command), this processor does not
 * write query output to stdout; instead it uses the query results to restrict the set of targets
 * that the build execution phase will build.
 *
 * <p>Code sharing with {@link CqueryProcessor}: both extend {@link PostAnalysisQueryProcessor} and
 * delegate to the same {@link ConfiguredTargetQueryEnvironment} setup.
 */
public final class BuildCqueryProcessor extends PostAnalysisQueryProcessor<CqueryNode> {
  ...
  /**
   * Evaluates the cquery expression against the post-analysis graph, then returns a copy of {@code
